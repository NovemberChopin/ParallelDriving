#include "../include/parallel_driving/dashboard.hpp"
...
Dashboard::Dashboard(QWidget *parent) :
    QWidget(parent)
{
    m_background = QColor(3, 15, 27);
//    m_foreground = Qt::white;
    m_foreground = QColor(0, 212, 222);
...
void Dashboard::drawCrown(QPainter *painter)
{
    ...
    lg1.setColorAt(0, QColor(253, 255, 169));   //设置渐变的颜色和路径比例
    lg1.setColorAt(1, QColor(192, 80, 30));    //只是粗略的颜色，具体的可以参考RGB颜色查询对照表
...
void Dashboard::drawScaleNum(QPainter *painter)
{
    ...
    for (int i = 0; i <= m_scaleMajor; i++)
    {
       ...
       tmpVal = 1.0 * i *((m_maxValue - m_minValue) / double(m_scaleMajor)) + m_minValue;
       // tmpVal = 50;
...
// 绘制刻度线
void Dashboard::drawScale(QPainter *painter) //绘制刻度线
{
    painter->save();
...
{   
    std::stringstream ss;
    ss << std::setiosflags(std::ios::fixed) << std::setprecision(2) << m_value;
    std::string str_0 = ss.str();
    QString str = QString("%1 %2").arg(QString::fromStdString(str_0)).arg(m_units);
